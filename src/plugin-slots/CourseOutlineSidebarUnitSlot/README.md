...
### Props:
* `unit`: The unit object, including its `title`, `icon`, `complete` status, etc.
...
This slot is used to replace/modify/hide the contents of an individual unit in the course outline sidebar. The
contents are rendered inside the unit's navigation link, so the slot replaces what is shown for the unit (such as its
...
The following `env.config.jsx` will replace the unit contents with a custom rendering.
